...
class SaveBestModel(LearnerCallback):
    ...
    def on_epoch_end(self, epoch: int, smooth_loss: Tensor, last_metrics: MetricsList, **kwargs: Any) -> bool:
        if self.config.genus:
            loss, genus_accuracy, species_accuracy, genus_f1_score, species_f1_score = last_metrics
            metric = (genus_f1_score+species_f1_score)/2
        else:
            loss, accuracy, f1_score = last_metrics

        if self.best_acc is None or accuracy > self.best_acc:
            self.best_acc = accuracy
            self.learn.save(f'best_acc' + self.outfile)
        if self.best_f1 is None or f1_score > self.best_f1:
            self.best_f1 = f1_score
            self.learn.save(f'best_f1' + self.outfile)
        if self.best_loss is None or loss < self.best_loss:
            self.best_loss = loss
            self.learn.save(f'best_loss' + self.outfile)
